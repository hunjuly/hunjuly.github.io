...
그러나 객체지향을 제대로 이해하는 프로그래머가 많지 않아서 객체지향 환경을 찾는 것은 쉽지 않다.\
...
마틴 파울러가 이야기하는 뇌의 구조가 바뀌는 순간을 나도 경험했다.\
나는 아래의 코드를 어떻게 하면 개선할 수 있을까를 고민하다가 객체지향의 기본을 깨닫게 됐다.\
...
이런 코드가 한 곳에서만 나타나면 문제가 없다. 그러나 국가를 판별해야 하는 코드는 곳곳에 있다.
이런 상황에서 국가가 추가되면 어떻게 될까?
여러 소스코드를 찾아가면서 하나씩 추가해야 한다.

어떻게 하면 해결할 수 있을까?
아래의 코드는 국가를 클래스로 나타냈다.
이제 국가가 추가돼도 그에 해당하는 클래스를 개발하기만 하면 된다.
클래스를 사용하는 쪽에서는 `interface Country`만 알면 되고 그것이 Kor인지 Eng인지는 신경쓰지 않는다.
조건문을 사용하지 않게 되는 것이다.
...
이 아이디어를 떠올린 그 순간 뇌의 구조와 사고방식이 바뀐 것을 느꼈다.
단순히 객체지향에 대한 의문 뿐만 아니라 그 동안 고민했던 좋은 코드에 대한 의문들이 상당 부분 해소됐다.
그 역시 객체지향이 개발 방법의 근간이기 때문일 것이다.
