...
사용자에게 위/아래/좌/우를 선택할 수 있는 방향키를 제공해야 한다.\
...
만약 화살표가 특정한 물체를 가리키는 용도라면 물체가 움직일 때 화살표를 회전시켜서 표현하는 것이 맞다.\
그러나 여기서는 위/아래/좌/우 각각 다른 의미를 나타내는 화살표다.\
화살표의 모양이 동일한 것은 그저 우연일 뿐이며 <그림 2-3>처럼 이미지가 달라질 수도 있다.

이번에는 <그림 2-4>를 보자.\
만약 Actor가 좌/우 둘 중 한 군데에 위치한다고 하면 어떨까?\
<그림 2-1>처럼 좌/우/ 화살표를 하나씩 사용하는 것이 좋을까?\
이렇다 하더라도 Actor를 가리키는 화살표의 의미가 변하는 것은 아니다.\
좌/우는 우연히 그렇게 결정된 것이며 Actor 위치는 언제든지 바뀔 수 있다.

<img src="/assets/refs/arrow-type2.png" />

간단하게 설명했지만 중요한 개념이다.
설계 단계에서 고민하게 되는 많은 것들은 이와 같이 분석 단계를 제대로 거치지 않은 경우가 많다.
잘못된 분석을 기반으로 설계와 구현을 해도 당장은 문제가 드러나지 않는다.
그러나 프로젝트가 진행될수록 요구사항이 구체화 되면 점점 설계와 맞지 않는다는 것을 알게된다.
이런 경우 설계와 구현을 변경해야 하는데 그렇게 하지 않으면 보통 나중에 더 큰 비용을 치루기 때문이다.
설계와 구현을 변경하는 과정에서 많은 테스트가 무효화 된다.

요구사항의 본질은 제쳐두고 메모리를 줄이느냐 읽기 쉬운 코드냐와 같은 기술적인 조건만을 가지고 판단하면 제대로 분석이 이루어지지 않는다.
