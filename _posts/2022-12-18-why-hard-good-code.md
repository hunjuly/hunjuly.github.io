...
분석이 어려운 이유 중 하나는 요구사항이 복잡하고 추상적이기 때문이다.
그래디 부치의 설명을 읽어보자.
...
비록 요구사항 분석이 어렵기는 하지만 시간을 들여서 프로토타입도 만들고 애자일 방법론을 적용하면 어떻게든 극복 할 수 있다.\
문제는 이런 분석을 통해서 알게된 요구사항을 어떻게 모델링 하느냐다.\
여기서 모델링은 요구사항 혹은 도메인을 코드로 표현하는 것을 뜻한다.\
예를 들어 '과일 마켓 서비스'를 만든다고 하면 과일의 종류/가격/생산일 정도를 코드로 표현하게 된다.\
과일의 성분/유래/효능은 '과일 마켓 서비스'의 관심사항이 아니기 때문에 모델링에 포함하지 않는다.

모델링의 어려움과 중요성을 설명해줄 경험을 공유한다.
...
사용자에게 위/아래/좌/우를 선택할 수 있는 화면을 제공해야 한다.\
...
```html
<Up src="arrow.png" rotate="0" />
<Left src="arrow.png" rotate="-90" />
<Right src="arrow.png" rotate="90" />
<Down src="arrow.png" rotate="180" />
```

![arrows](/assets/refs/arrows.png)

어떤 방법이 옳은 것일까? <그림 2-1>의 방법은 코드를 읽기 쉽고 <그림 2-2>의 방법은 이미지의 크기를 줄일 수 있다.\
다양한 생각들이 있겠지만 우선적으로 고려해야 하는 것은 4개 화살표의 의미다.\
만약 화살표가 특정한 물체를 가리키는 용도라면 하나의 화살표를 회전시켜서 표현하는 것이 맞다.\
그러나 여기서는 위/아래/좌/우를 나타내는 각각 다른 의미의 화살표다.\
화살표의 모양이 동일한 것은 그저 우연일 뿐이며 디자이너의 생각에 따라서 4개의 이미지가 달라질 수도 있다.

<p align="center">

<img src="/assets/refs/arrow-type2.png" alt="drawing" width="200"/>
<figcaption align = "center"><b>이런 화살표는 회전시켜서 사용할 수 없다.</b></figcaption>
</p>

<!-- ![arrows](/assets/refs/arrow-type2.png) -->

> 위/아래 좌/우 화살표의 모양이 똑같다. 하나의 화살표 이미지를 90도씩 회전시켜서 사용하는게 좋을까? 회전된 이미지 4개를 사용하는 것이 좋을까?
> 우연히 4개의 모양이 같을 뿐이지 의미는 각각 다르다. 만약 화살표가 왼쪽/오른쪽이 아니라 특정한 물체를 가리키는 것이고 물체는 왼쪽이나 오른쪽에 있을 수 있다면 이것은 하나의 화살표를 사용하는 것이 맞다. 왼쪽/오른쪽은 우연일 뿐이고 본질은 물체를 가리키는 화살표이기 때문이다.
