...
> 만약 분리하는 것이 부담스러워서 리팩토링을 피하려고 했다면 코드 곳곳에 if-else가 넘쳐나고 지옥으로 가는 문이 열렸을 것이다.
...
많은 프로그래머들이 이 어려움을 극복하지 못하고 노력을 중단하는 것을 보아왔다.\
...
그러나 올바른 결정을 할 수 없을 때도 스타일로 치부하는 경향이 있다.\
