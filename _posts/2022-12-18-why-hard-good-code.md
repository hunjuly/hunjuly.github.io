...
어떻게 해야 하는가? - 분석부터 제대로
 -->
좋은 코드를 작성하고 싶지만 무엇이 좋은 코드이고 어떻게 해야 할지 모르는 경우가 많다.\
널리 알려진 소스코드를 보면 어딘지 모르게 깔끔하고 읽기가 쉬운데 내가 작성한 코드는 아무리 노력해도 어설프다.\
그래서 리팩토링을 공부하고 클린코드 같은 책도 읽어보지만 크게 도움이 되지 않는다.
...
그리고 얼마 지나지 않아서 아래의 순서로 좌절하게 된다.

1. 쓸데없이 복잡한 클래스를 변경하게 된다.
2. 클래스가 변경되면서 무효화된 테스트를 삭제하게 된다.
3. 프로젝트는 진행되지 않고 리팩토링만 반복하게 된다.

이 정도 노력해서 안 되면 대부분 포기하게 되고 좋은 코드는 책에서나 나오는 이상론이 되어버린다.
그러나 이상론으로 치부하기에는 현대적인 개발 프레임워크들은 모두 테스트 자동화를 충실히 지원하고 있다.
그리고 코드가 테스트 자동화를 성공적으로 유지하고 있다면 다른 부분도 꽤 좋은 코드일 확률이 높다.

> 실무에서 좋은 코드를 이상론으로 치부하는 부류가 같이 일하기에 힘든 편이다.
> 이렇게 하면 된다는 얘기를 해도 믿을 용기가 부족하기 때문이다.
> 주로 경력이 10년 넘은 평범한 실력의 프로그래머들이 그렇다.
