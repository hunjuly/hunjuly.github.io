...
사용자에게 위/아래/좌/우를 선택할 수 있는 방향키를 제공해야 한다고 가정하자.\
...
그러나 여기서는 위/아래/좌/우 개념을 나타내는 각각 다른 의미의 화살표이며 화살표의 모양이 동일한 것은 그저 우연일 뿐이다.\
<그림 2-3>처럼 이미지가 달라지면 회전시켜서 구현하는 방법은 맞지 않게 된다.

<img src="/assets/refs/arrow-type2.png" />

짧은 설명이지만 중요한 개념이다.\
해석은 분석에서 설계로 넘어가는 애매한 위치에 있어서 그런지 그 개념과 존재가 비중있게 다뤄지지 않는 것 같다.

설계 단계에서 고민하게 되는 원인 중 상당수는 잘못된 해석에 기인한다.\
잘못된 분석을 기반으로 설계를 하면 얼마 안 가서 문제가 드러난다.\
그러나 잘못된 해석을 기반으로 하는 설계는 당장 문제가 되지는 않는다.\
그러나 프로젝트가 진행될수록 요구사항이 구체화 되면서 점점 설계와 맞지 않는 개념이 드러난다.\
이런 경우 설계와 구현을 변경해야 하는데 그렇게 하지 않으면 보통 나중에 더 큰 비용을 치루게 된다.\
설계와 구현을 변경하는 과정에서 많은 테스트가 무효화 된다.

<!-- 면밀한 분석은 당연하고 예리한 해석이 없다면.. -->
