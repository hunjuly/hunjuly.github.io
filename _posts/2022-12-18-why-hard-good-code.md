...
거기에 더 많은 노력으로 다양한 디자인패턴과 TDD 같은 개발 방법까지 습득했다면 소프트웨어를 잘 설계할 수 있지 않을까?
...
분석을 중요하게 다루는 책은 설계 만큼 많지는 않은 것 같다.\
그나마 DDD가 가장 가까운 편이고 나도 또한 DDD를 읽으면서 어떻게 분석해야 하는지를 깨달았다.
요구사항 분석이 뭐 어려울까 싶기는 하다. 예전 일을 공유한다.
...
그러나 구현을 하지 못하면 설계를 못하고 설계를 못하면 분석을 못한다. 그래서 보통은 구현 -> 설계 -> 분석의 순서로 학습을 하게된다.
