...
코드의 집합체인 소프트웨어가 자주 변해야 하기 때문인데 그 이유가 무엇인지 알아보자.

## 소프트웨어가 변화해야 하는 이유

소프트웨어는 개발의 시작부터 운영의 종료까지 끊임없이 변화를 요구받는다.
여기서 변화는 소프트웨어 그 자체이기도 하고 외부 환경이기도 하다.

### 소프트웨어의 유연성
소프트웨어가 가지는 유연성이라는 본질적 특성이 문제의 근원이다.
많은 책에서 유연성으로 정의해서 나도 그렇게 언급하지만 무형이라는 특성이 더 맞을 것 같다.
무형이기 때문에 변경하기 쉬운 만큼 구체화 하기도 어렵다.
건축물은 지어질 집을 3D모델링도 하고 미니어쳐로 만들어 보기도 한다.
