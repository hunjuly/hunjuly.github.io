...
다시 말하면 소프트웨어는 복잡해서 인간이 시행착오 없이 만들 수는 없다. 점진적인 개선을 통해서 완성해 나가는데 그래서 변경이 계속되는 것이다.\

> 산업용 소프트웨어 특징으로는 개발자 한 사람이 설계 부분까지 상세히 모두 알 기에는 무척 까다롭다.
...
그 중 리팩토링은 좋은 시작점인데, 현재 코드의 무엇이 문제이고 이것을 어떻게 개선하면 되는지에 대한 이야기다. `Martin Fowler`의 `Refactoring`을 추천한다.
...
위의 개발 방법과 도구들의 공통점은 변화에 대응한다는 것이다.\
변화의 대상을 정의하고 그 변화에 독립적인 솔루션을 제시한다.\
그것은 코드도 마찬가지인데 좋은 코드는 변화에 쉽게 적응한다.\
이런 코드의 특징은 의존성이 낮고 응집성이 높은 것이다.
...
그리고 다양한 좋은 코드에 대해 소개하려고 하는 이 블로그 또한 응집성과 의존성에 대한 이야기다.\
