...
이 글은 좋은 코드가 무엇이고 왜 그렇게 됐는지에 대한 나의 생각이다.

프로그래머의 실력에 한정해서 이야기하는 것에 주의하기를 바란다.\
...
왜 프로그래머들은 변경하기 쉬운 코드를 좋은 코드라고 생각할까?
코드의 집합체인 소프트웨어를 자주 변경해야 하기 때문이다.
그리고 그것은 결국 프로그래머 입장에서 다른 무언가가 변했기 때문인데 주로 사용자의 요구사항이 그렇다.
그 외에 종종 운영 환경이 변경되기도 하는데 몇 가지 예를 들면 아래와 같다.
...
# 요구사항이 변화하는 이유
운영환경은 어쩔 수 없다고 해도 요구사항은 왜 계속 변경될까?
무엇을 만들 것인지 신중하게 분석하고 설계하면 시행착오를 줄일 수 있지 않을까?
