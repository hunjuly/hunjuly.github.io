...
사람은 누구나 자기가 노력하는 분야에서 실력으로 인정받기를 원하고 프로그래머도 마찬가지다.\
그러나 프로그래밍 실력의 기준은 제각각이다.\
누군가는 알고리즘 같은 기본기를 중요시 하고 누군가는 빠르게 기능을 구현하는 것이 실력의 기준이 된다.\
거기에 디버깅도 잘 하고 최신 기술도 많이 알면 보통은 실력있는 프로그래머로 인정 받는 것 같다.

그러나 그게 전부일까?\
내 생각에 SDK의 문서를 읽고 기능을 구현하는 것은 당연하고 평범한 기술이다.\
알고리즘도 몇 개 정도를 이해하면 나머지는 개인의 재능에 따른 응용일 뿐이다.\
과연 주니어 프로그래머와 시니어 프로그래머는 이런 기술들이 얼마나 있느냐의 차이일 뿐인가?

나는 `프로그래머의 실력은 얼마나 좋은 코드를 작성할 수 있는가`에 달렸다고 생각한다.\
물론 좋은 코드라는 표현은 추상적이고 주관적이다.\
그럼에도 불구하고 내가 생각하는 좋은 코드가 무엇이고 왜 그렇게 됐는지 얘기하려고 한다.

실력있는 프로그래머로 한정해서 이야기하는 것에 주의하기를 바란다.\
좋은 프로그래머라면 실력 뿐만이 아니라 커뮤니케이션 능력을 포함하는 현명함도 갖추어야 한다.\
나 자신이 좋은 프로그래머라는 확신이 없고 정답도 존재할 것 같지 않다.\
그러니 이에 대한 고민은 각자가 평생에 걸쳐서 노력해야 할 것이다.
...
코드의 집합체인 소프트웨어가 자주 변해야 하기 때문이다.

변해야 하는 이유는 다양한데 개발 도중이라면 요구사항이 계속 변경되고 출시를 한 후에도 기능 개선은 계속된다.
기능을 개선하지 않더라도 운영 환경이 변하게 되는데 몇 가지 예를 들면 아래와 같다.
...
이와 같이 어떤 식으로든 변경은 발생하고 기능 개선의 요구도 끝없이 나오는 것이 일반적이다.

## 소프트웨어가 변화하는 이유
운영환경은 어쩔 수 없다고 해도 (보통은 큰 장애가 되지 않기도 한다.)
우리에게 가장 큰 고통을 주는 요구사항이 계속 변경되는 이유는 무엇일까?

### 소프트웨어의 유연성

소프트웨어라는 이름에서 알 수 있듯이 소프트웨어의 유연성은 본질적 특성이다.
...
잦은 변경의 과정을 거쳐서 출시를 해도 소프트웨어의 운영 환경이 계속 변하게 된다.
하드웨어는 소프트웨어를 담는 그릇이라고 할 수 있는데 하드웨어의 발전이 빠르면 그 만큼 소프트웨어도 빠르기 마련이다.
