...
그리고 그것은 결국 다른 무언가가 변했기 때문인데 주로 사용자의 요구사항이 그렇다.\
...
여기서 얘기하는 요구사항의 변경은 `사용자가 제시하는 요구사항의 변경` 뿐만 아니라 제시된 요구사항을 `분석 단계에서 잘못 이해하는 것`을 포함한다.

운영환경은 어쩔 수 없다고 해도 요구사항은 왜 계속 변경될까?
무엇을 만들 것인지 신중하게 분석하고 설계하면 시행착오를 줄일 수 있지 않을까?하는 아쉬움이 있을 것이다.
그러나 이 정도 되면 그냥 특성으로 받아들여야 한다.
